...
class EXU extends Module {
  ...
  io.em_bus.data_sram_en    := de_bus_r.res_from_mem && es_valid
  io.em_bus.data_sram_we    := de_bus_r.MemWen && es_valid
  io.em_bus.data_sram_addr  := result
  io.em_bus.data_sram_wdata := src2
  io.em_bus.data_sram_wmask := de_bus_r.wmask
  ...
  es_ready_go       := Mux(mul_end || div_end,true.B,Mux(muling || diving || reming,false.B,true.B))
