...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  //RISC-V 64-bit Instruction Set Manual v2.2
  //R-type 
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = src2 == 0 ? 0 : (sword_t)src1 / (sword_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src2 == 0 ? 0 : src1 / src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = SEXT(BITS(src2,31,0) == 0 ? 0 : (sword_t)BITS(src1,31,0) / (sword_t)BITS(src2,31,0),32));
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(dest) = SEXT(BITS(src2,31,0) == 0 ? 0 : BITS(src1,31,0) / BITS(src2,31,0),32));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT(BITS((src1 << BITS(src2,5,0)),31,0), 32));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) = SEXT((sword_t)BITS(src2,31,0) == 0 ? 0 : (sword_t)BITS(src1,31,0) % (sword_t)BITS(src2,31,0),32));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(dest) = SEXT(BITS(src2,31,0) == 0 ? 0 : BITS(src1,31,0) % BITS(src2,31,0),32));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> (sword_t)BITS(imm, 5, 0));
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | imm);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & imm);
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT(BITS((src1 << BITS(imm,5,0)),31,0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT((int32_t)BITS(src1,31,0) >> BITS(imm,5,0), 32));
