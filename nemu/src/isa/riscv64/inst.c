...
static vaddr_t *csr_reg(word_t imm){
  switch(imm){
    ...
    default:panic("csr_reg: unknown csr register %lx", imm);
  }
}
#define ECALL(dnpc){bool success;difftest_skip_ref();dnpc = (isa_raise_intr(isa_reg_str2val("a7", &success), s->pc));}
#define CSR(i) *csr_reg(i)
#define MRET(dnpc){dnpc = CSR(0x341)+4;}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = R(dest) = SEXT(src1, 32) / SEXT(src2, 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = src1 >> BITS(src2,5,0));
